...
import { uniqWith, differenceWith, zip, partition, first, omit } from "lodash";
import {
...
const main = async () => {
  ...
  const res = await getVolumeStats();

  const uniqueAnchors = uniqWith(
    res.converters.map(converter => converter.anchor),
    compareString
  );
  ...
  const volumes = groupedByAnchors.map(group =>
    group.converters.flatMap(converter =>
      converter.volumes.find(volume => compareString(volume.token.id, bntToken))
    )
  );
  console.log(
    groupedByAnchors,
    "are grouped by anchors",
    volumes,
    "was volumes"
  );
  ...
  const totalVolume = filteredVolumes.reduce((acc, item) =>
